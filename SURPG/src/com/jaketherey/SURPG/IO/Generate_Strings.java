		...
		while(in.hasNextLine()) {
			...
			for(int i = 0; i < answer_bits.length; i++) {
				...
				try{
					...
					answerList.add(new Answer_Packet(packetBits[0], Integer.parseInt(packetBits[1]), runList.toArray(new Answer_Runs[runList.size()])));
					
				}catch(IndexOutOfBoundsException | NullPointerException | SecurityException e) {
					
					answerList.add(new Answer_Packet(packetBits[0], 0, new Answer_Runs[0]));
